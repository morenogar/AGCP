...
void NormalMapApp::Update(const GameTimer& gt)
{
    OnKeyboardInput(gt);
		...
		XMMATRIX gridRitem2lRot = XMMatrixRotationX(1.55f);
...
void NormalMapApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for(auto& e : mAllRitems)
	{
		...
		if(e->NumFramesDirty > 0)
		{
			...
			objConstants.ObjPad0 = e->parallax;
...
void NormalMapApp::UpdateMaterialBuffer(const GameTimer& gt)
{
	...
	for(auto& e : mMaterials)
	{
		...
		if(mat->NumFramesDirty > 0)
		{
			...
			matData.DepthMapIndex = mat->NormalSrvHeapIndex;
...
void NormalMapApp::BuildDescriptorHeaps()
{
	...
	std::vector<ComPtr<ID3D12Resource>> tex2DList = 
	{
		...
		mTextures["bricksDepthMap"]->Resource,
...
void NormalMapApp::BuildRenderItems()
{
	...
	XMMATRIX gridRitemlWorld = XMMatrixTranslation(0.0f,0.0f,0.0f);


	//PARALLAX
	auto gridRitem = std::make_unique<RenderItem>();
	gridRitem->World = MathHelper::Identity4x4();
	XMStoreFloat4x4(&gridRitem->TexTransform, brickTexTransform);
	XMStoreFloat4x4(&gridRitem->World, XMMatrixScaling(0.5f, 0.5f, 0.4f) * gridRitemlWorld );
	gridRitem->ObjCBIndex = 1;
	gridRitem->parallax = 1;
